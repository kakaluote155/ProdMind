...
<head>
  ...
  <style>
    ...
    * { box-sizing: border-box; }
    ...
    input, button { border-radius: 9px; border: 1px solid #33415f; padding: 10px 11px; font: inherit; }
    input { background: #0d1427; color: #fff; min-width: 0; }
...
<header>
  ...
  <p>Inspect how current evidence supports a root-cause diagnosis.</p>
...
<script>
  ...
  function drawEdges(edges, nodes, width, height) {
    ...
    edges.forEach(edge => {
      ...
      path.setAttribute('d',`M ${x1} ${y1} C ${mid} ${y1}, ${mid} ${y2}, ${x2-8} ${y2}`); path.setAttribute('fill','none'); path.setAttribute('stroke','#64748b'); path.setAttribute('marker-end','url(#arrow)'); edgesEl.appendChild(path);
